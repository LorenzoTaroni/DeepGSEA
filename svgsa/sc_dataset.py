...
class SingleCellDataset(Dataset):
    def __init__(self, adata, n_gs, normalize = True):
        if normalize:
            ...
            self.data = adata.X.copy()
            self.data.data = np.clip(self.data.data.astype(np.float32), -10, 10)
            self.raw = adata.raw
        else:
            self.data = np.clip(adata.X.astype(np.float32), -10,10)
            self.raw = adata.raw
        #self.counts = torch.tensor(self.data).unsqueeze(0).repeat(n_gs, 1, 1)
        self.counts = torch.tensor(self.data.toarray() if issparse(self.data) else self.data).unsqueeze(0).repeat(n_gs, 1, 1)
